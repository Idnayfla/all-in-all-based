...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (projectType === 'python') {
      const main =
        files.find((f: any) => f.name === 'main.py') ??
        files.find((f: any) => f.name.endsWith('.py'));
      ...
      const javaFiles = files
        .filter((f: any) => f.name.endsWith('.java'))
        .map((f: any) => f.name)
        .join(' ');
      ...
      if (compile.stderr?.trim()) {
        ...
        const main =
          files.find((f: any) => f.name === 'Main.java') ??
          files.find((f: any) => f.name.endsWith('.java'));
      ...
      const entry =
        files.find((f: any) => f.name === 'main.cpp') ??
        files.find((f: any) => f.name.endsWith('.cpp'));
      ...
      const entry =
        files.find((f: any) => f.name === 'main.go') ??
        files.find((f: any) => f.name.endsWith('.go'));
      ...
      const entry =
        files.find((f: any) => f.name === 'main.rs') ??
        files.find((f: any) => f.name.endsWith('.rs'));
