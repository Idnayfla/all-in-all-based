...
const DEFINITIONS = [
  ...
  },
  {
    name: 'run_command',
    description:
      'Run any shell command in the project root — git, npm, python, curl, anything. 60s timeout.',
    input_schema: {
      type: 'object',
  ...
  // ── New: Web & Network ─────────────────────────────────────────────────────
  {
    name: 'fetch_url',
    description:
      'Make any HTTP request — GET a web page, call an API, POST data, hit a webhook. Returns the response body.',
    input_schema: {
      type: 'object',
      properties: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        action: {
          ...
          description: 'Single action shorthand. Ignored if steps is provided.',
        },
        selector: {
          type: 'string',
          description: 'CSS selector for single-action click/fill/extract.',
        },
        ...
        steps: {
          type: 'array',
          description:
            'Run multiple actions in ONE browser session. Each step: {action, selector?, text?, wait?}. Use this for any multi-step flow (login, navigate, fill form, read result).',
          items: {
            type: 'object',
            properties: {
              action: {
                type: 'string',
                enum: ['read', 'screenshot', 'click', 'fill', 'extract', 'wait'],
              },
  ...
  // ── New: Communication ─────────────────────────────────────────────────────
  {
    name: 'send_email',
    description:
      'Send an email. Requires email config in config.json (email.smtp_host, email.user, email.pass).',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'notify_desktop',
    description:
      'Send an immediate desktop notification to Hus. Use for urgent alerts that need attention now.',
    input_schema: {
      type: 'object',
      properties: {
        title: { type: 'string' },
  ...
  // ── New: GitHub ────────────────────────────────────────────────────────────
  {
    ...
    description:
      'Create a GitHub pull request from the current branch. Requires gh CLI authenticated.',
    input_schema: {
      type: 'object',
      properties: {
        title: { type: 'string' },
  ...
  // ── New: System ───────────────────────────────────────────────────────────
  {
    ...
    input_schema: {
      ...
      properties: {
        type: {
          type: 'string',
          enum: ['memory', 'cpu', 'disk', 'processes', 'all'],
  ...
  // ── New: Discord file sending ─────────────────────────────────────────────
  {
    name: 'send_file',
    description:
      'Send an image, video, GIF, PDF, or any file directly into the Discord conversation — exactly like a human would. Accepts a local file path OR a public URL. Optionally include a caption.',
    input_schema: {
      type: 'object',
      properties: {
        file: {
          type: 'string',
          description: 'Local file path (relative to project root) OR a public URL (https://...).',
        },
  ...
  },
  {
    name: 'search_gif',
    description:
      'Search Tenor for a GIF to share in Discord. Use for celebrations, reactions, or when a GIF fits better than words. Returns a URL — pass it to send_file to post it.',
    input_schema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description:
            'What to search for, e.g. "celebrating", "mind blown", "good job", "facepalm"',
        },
      },
      required: ['query'],
    },
  },
  {
    name: 'dm_hus',
    description:
      'Send a private direct message to Hus. Use for sensitive info, a private heads-up, or anything not right for the main channel.',
    input_schema: {
      type: 'object',
  ...
  // ── Work efficiency ────────────────────────────────────────────────────────
  {
    name: 'stripe_query',
    description:
      'Query Stripe for real revenue data — MRR, active subscriptions, recent charges, failed payments. Requires stripe_secret_key in config.json.',
    input_schema: {
      type: 'object',
      properties: {
        type: {
          type: 'string',
          enum: ['mrr', 'subscriptions', 'charges', 'failed', 'summary'],
          description:
            'mrr=monthly recurring revenue, subscriptions=active sub list, charges=recent payments, failed=failed charges, summary=MRR+sub count+recent.',
        },
        limit: {
          type: 'number',
  ...
  },
  {
    ...
    description:
      'Schedule a Discord message to be sent at a specific future time. Use to set reminders, follow-ups, or time-based announcements. The message will appear in the current channel from you.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        send_at: {
          type: 'string',
          description:
            'ISO 8601 datetime when to send, e.g. "2025-06-10T09:00:00+08:00" for 9am SGT.',
        },
        channel: {
          type: 'string',
          description: 'Channel name to post in (default: current channel).',
        },
      },
  ...
  },
  {
    ...
    description:
      'Read recent message history from any channel — catch up on what was discussed, get context, review decisions made while you were away.',
    input_schema: {
      type: 'object',
      properties: {
        channel_name: {
          type: 'string',
          description: 'Channel name to read from (e.g. "council", "senior-engineer").',
        },
        limit: {
          type: 'number',
...

  {
    name: 'github_comment',
    description:
      'Leave a comment on a GitHub issue or pull request. Use for code review feedback, status updates, or responding to discussions.',
    input_schema: {
      type: 'object',
      properties: {
        type: {
          type: 'string',
          enum: ['issue', 'pr'],
          description: 'Whether to comment on an issue or PR.',
        },
  ...
  },
  {
    ...
    description:
      'Update a GitHub issue — close it, reopen it, change the title, add/remove labels, or assign it. Core triage and project management tool.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        action: {
          type: 'string',
          enum: ['close', 'reopen', 'label', 'unlabel', 'assign', 'unassign', 'title', 'body'],
          description: 'What to do.',
        },
        value: {
          type: 'string',
          description:
            'The value for the action — label name, assignee username, new title, or new body.',
        },
      },
  ...
  // ── Previously added ───────────────────────────────────────────────────────
  {
    name: 'github_read',
    description:
      "Read GitHub issues, pull requests, PR diffs, and review status for this repo. Use to check what's open, review code, or see what needs attention.",
    input_schema: {
      type: 'object',
      properties: {
        type: {
          type: 'string',
          enum: ['issues', 'prs', 'issue', 'pr_diff', 'pr_reviews'],
          description:
            'issues=open issue list, prs=open PR list, issue=single issue detail, pr_diff=diff of a PR, pr_reviews=review status of a PR.',
        },
        number: {
          type: 'number',
          description: 'Issue or PR number (required for issue, pr_diff, pr_reviews).',
        },
        state: {
          type: 'string',
  ...
  },
  {
    name: 'agent_notes',
    description:
      'Your personal persistent scratchpad. Save observations, todos, open questions, and decisions you want to remember across sessions. Separate from auto-extracted memory — this is what YOU deliberately choose to track.',
    input_schema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['save', 'read', 'delete'],
          description: 'save=add/update a note, read=get all your notes, delete=remove a note.',
        },
        key: {
          type: 'string',
          description:
            'Short note identifier, e.g. "open-question-auth" or "todo-perf-audit" (required for save and delete).',
        },
  ...
  },
  {
    name: 'posthog_query',
    description:
      'Query PostHog analytics for Based — DAUs, top events, specific event counts. Requires posthog_api_key and posthog_project_id in config.json.',
    input_schema: {
      type: 'object',
      properties: {
        type: {
          type: 'string',
          enum: ['dau', 'events', 'custom'],
          description:
            'dau=daily active users trend, events=top event counts, custom=specific event over time.',
        },
        event: {
          type: 'string',
          description: 'Event name for custom query (e.g. "generation_started", "$pageview").',
        },
  ...
  },
  {
    ...
    description:
      'Create a scheduled event in the Discord server — team meetings, reviews, syncs. Appears in the Events tab so everyone can see it.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        start_time: {
          type: 'string',
          description: 'ISO 8601 datetime, e.g. "2025-06-10T14:00:00+08:00" for 2pm SGT.',
        },
...
function describeUse(name, input) {
  switch (name) {
    ...
    case 'get_git_info':
      return `Checking git ${input.type}`;
    case 'web_search':
      return `Searching: "${(input.query || '').slice(0, 60)}"`;
    case 'consult_agent':
      return `Consulting ${input.agent}`;
    case 'create_github_issue':
      return `Creating issue: "${(input.title || '').slice(0, 60)}"`;
    case 'fetch_url':
      return `Fetching ${input.method || 'GET'} ${(input.url || '').slice(0, 80)}`;
    case 'browse_web':
      return `Browser ${input.action} → ${(input.url || '').slice(0, 60)}`;
    case 'download_file':
      return `Downloading to \`${input.destination}\``;
    case 'send_email':
      return `Sending email to ${input.to}`;
    case 'notify_desktop':
      return `Notifying: "${(input.title || '').slice(0, 50)}"`;
    case 'create_github_pr':
      return `Creating PR: "${(input.title || '').slice(0, 60)}"`;
    case 'manage_files':
      return `${input.action} \`${input.source}\``;
    case 'get_system_info':
      return `System ${input.type}`;
    case 'send_file':
      return `Sending file: ${(input.file || '').slice(0, 60)}`;
    case 'search_gif':
      return `Searching GIF: "${(input.query || '').slice(0, 40)}"`;
    case 'dm_hus':
      return `DMing Hus`;
    case 'github_comment':
      return `GitHub comment on ${input.type} #${input.number}`;
    case 'update_github_issue':
      return `GitHub issue #${input.number}: ${input.action}${input.value ? ` → ${input.value}` : ''}`;
    case 'stripe_query':
      return `Stripe ${input.type}`;
    case 'schedule_message':
      return `Scheduling message for ${(input.send_at || '').slice(0, 16)}`;
    case 'read_discord_history':
      return `Reading #${input.channel_name} history (${input.limit || 20} msgs)`;
    case 'github_read':
      return `GitHub ${input.type}${input.number ? ` #${input.number}` : ''}`;
    case 'agent_notes':
      return `Notes: ${input.action}${input.key ? ` "${input.key}"` : ''}`;
    case 'posthog_query':
      return `PostHog ${input.type}${input.event ? ` (${input.event})` : ''} ${input.days || 7}d`;
    case 'create_discord_event':
      return `Creating event: "${(input.name || '').slice(0, 40)}"`;
    default:
      return `Using \`${name}\``;
  }
}
...
function safePath(p) {
  ...
  if (!abs.startsWith(path.resolve(PROJECT_ROOT))) throw new Error('Path outside project root.');
  return abs;
}
...
async function execute(name, input, context) {
  try {
    switch (name) {
      ...
      case 'consult_agent':
        return await consultAgent(input, context);
      case 'read_file':
      ...
      case 'search_codebase':
        return searchCodebase(input);
      case 'list_files':
        return listFiles(input);
      case 'get_git_info':
        return getGitInfo(input);
      case 'create_github_issue':
        return await createGithubIssue(input);
      // New
      case 'fetch_url':
        return await fetchUrl(input);
      case 'browse_web':
        return await browseWeb(input);
      case 'download_file':
        return await downloadFile(input);
      case 'send_email':
        return await sendEmail(input);
      case 'notify_desktop':
        return notifyDesktop(input);
      case 'create_github_pr':
        return createGithubPr(input);
      case 'manage_files':
        return manageFiles(input);
      case 'get_system_info':
        return getSystemInfo(input);
      case 'send_file':
        return await sendFile(input, context);
      case 'search_gif':
        return await searchGif(input);
      case 'dm_hus':
        return await dmHus(input, context);
      case 'github_comment':
        return githubComment(input);
      case 'update_github_issue':
        return updateGithubIssue(input);
      case 'stripe_query':
        return await stripeQuery(input);
      case 'schedule_message':
        return await scheduleMessage(input, context);
      case 'read_discord_history':
        return await readDiscordHistory(input, context);
      case 'github_read':
        return githubRead(input);
      case 'agent_notes':
        return agentNotes(input, context);
      case 'posthog_query':
        return await posthogQuery(input);
      case 'create_discord_event':
        return await createDiscordEvent(input, context);
      default:
...
async function consultAgent({ agent, question }, context) {
  ...
  if (!AGENTS[slug])
    return `Unknown agent: ${agent}. Valid slugs: ${Object.keys(AGENTS).join(', ')}`;
  ...
  const targetChannel =
    guild?.channels.cache.find(c => c.name === agent_slug && c.isTextBased?.()) || context.channel;
  ...
  const isBad =
    !reply?.trim() || reply.includes('[Max depth reached]') || reply.includes('Max depth');
  if (isBad) {
...
function getGitInfo({ type }) {
  ...
  try {
    return (
      execSync(cmds[type] || 'git status', { cwd: PROJECT_ROOT, encoding: 'utf-8' }).trim() ||
      '(clean)'
    );
  } catch (e) {
...
async function createGithubIssue({ title, body, labels = [] }) {
  ...
  try {
    ...
    return `Failed: ${(e.stderr || e.message || '').slice(0, 400)}`;
  }
}
      ...
      for (const step of steps) {
        ...
        try {
          if (sa === 'wait') {
            ...
            if (!ss) {
              results.push('click: selector required');
              continue;
            }
            await page.waitForSelector(ss, { timeout: 10000 });
            try {
              await page.locator(ss).first().click({ force: true, timeout: 8000 });
            } catch {
              await page.evaluate(s => document.querySelector(s)?.click(), ss);
            }
            await page.waitForTimeout(1500 + sw);
            results.push(`Clicked "${ss}"`);
          } else if (sa === 'fill') {
            if (!ss) {
              results.push('fill: selector required');
              continue;
            }
            await page.waitForSelector(ss, { timeout: 10000 });
            ...
            if (!ss) {
              results.push('extract: selector required');
              continue;
            }
            const extracted = await page
              .$$eval(ss, els => els.map(e => e.innerText).join('\n'))
              .catch(() => '');
    ...
    switch (action) {
      case 'read': {
        const { text, interactive } = await page.evaluate(() => {
          const text = document.body?.innerText || '';
          const els = [
            ...document.querySelectorAll('a, button, input, textarea, [role="button"], [onclick]'),
          ];
          const interactive = els
            .map(el => {
              const tag = el.tagName.toLowerCase();
...
function createGithubPr({ title, body, base = 'main', draft = false }) {
  ...
  try {
    ...
    try {
      execSync(`git push -u origin ${branch}`, { cwd: PROJECT_ROOT, encoding: 'utf-8' });
    } catch {}
...
function getSystemInfo({ type }) {
  switch (type) {
    ...
    case 'disk': {
      try {
        // Windows
        const out = execSync('wmic logicaldisk get Caption,Size,FreeSpace /format:csv', {
          encoding: 'utf-8',
          timeout: 5000,
        });
        ...
        try {
          ...
          return `Disk info unavailable: ${e.message}`;
        }
      }
    }
    case 'processes': {
      try {
        ...
        return `Process list unavailable: ${e.message}`;
      }
    }
    case 'all': {
      const total = os.totalmem();
...
async function searchGif({ query }) {
  ...
  if (!key)
    return 'Giphy not configured (get a free key at developers.giphy.com, add giphy_api_key to config.json)';
  try {
...
async function stripeQuery({ type, limit = 10 }) {
  ...
  if (!key)
    return 'Stripe not configured. Add stripe_secret_key to config.json (get from dashboard.stripe.com → Developers → API keys).';
    ...
    switch (type) {
      ...
      case 'subscriptions': {
        ...
        const rows = (subs.data || []).map(s => {
          ...
          const plan =
            s.items?.data?.[0]?.price?.nickname || s.items?.data?.[0]?.price?.id || 'plan';
...
async function scheduleMessage({ message, send_at, channel: channelName }, context) {
  ...
  const sgt = fireAt.toLocaleString('en-SG', {
...
async function readDiscordHistory({ channel_name, limit = 20 }, context) {
  ...
  if (!ch)
    return `Channel #${channel_name} not found. Available: ${guild.channels.cache
  ...
  try {
    ...
    return sorted
      .map(m => {
        const time = m.createdAt.toLocaleTimeString('en-SG', {
          timeZone: 'Asia/Singapore',
          hour: '2-digit',
          minute: '2-digit',
        });
...
async function posthogQuery({ type, event, days = 7 }) {
  ...
  if (!key)
    return 'PostHog not configured. Add posthog_api_key to config.json (get from posthog.com → Settings → Personal API keys).';
  if (!projectId)
    return 'PostHog not configured. Add posthog_project_id to config.json (visible in your PostHog project URL).';
...
async function createDiscordEvent(
  { name, description = '', start_time, duration_minutes = 60 },
  context
) {
  const discord = context?.discordClient;
  if (!discord) return 'No Discord client in context — works only during live conversations.';
  ...
  try {
    ...
    if (isNaN(start.getTime()))
      return `Invalid start_time: "${start_time}". Use ISO 8601 format, e.g. "2025-06-15T14:00:00+08:00".`;
    ...
    const sgt = start.toLocaleString('en-SG', {
      timeZone: 'Asia/Singapore',
      dateStyle: 'medium',
      timeStyle: 'short',
    });
