...
if (-not $nodeOk) {
    ...
    if ($nvmExists) {
        ...
        winget install --id OpenJS.NodeJS.20 --silent --accept-source-agreements --accept-package-agreements

        # Refresh PATH from registry so new Node is usable without restarting
        ...
        if (-not $nodeOk) {
            Write-Host "      Node upgrade failed. Close and reopen this terminal, then run setup again." -ForegroundColor Red
            exit 1
        }
    }
}
