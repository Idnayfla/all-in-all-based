#!/usr/bin/env python3
"""
Based Jarvis — Raspberry Pi voice companion
Say the wake word → speak → Based (or local Ollama) responds with voice

Wake word engines (priority order):
  1. openWakeWord  — free, offline, models: hey_jarvis / alexa / hey_mycroft
  2. pvporcupine   — supports custom wake words, requires Picovoice key
...
def init_wake_word(config):
    ...
    ww_name = config.get('wake_word', 'hey_jarvis')
...
# ── LLM: Ollama local ─────────────────────────────────────────────────────
JARVIS_SYSTEM = (
    "You are Based Jarvis — a sharp, direct AI voice companion running locally on a "
    "Raspberry Pi. Keep every response under 2 sentences unless the user explicitly asks "
    "for more detail. No bullet points, no markdown — plain conversational speech only."
)


def generate_ollama(text, history, config):
    """Call local Ollama (OpenAI-compatible), return full reply text."""
    ...
    try:
        ...
        return "I can't reach my local brain right now. Make sure Ollama is running."
...
def speak_local(text):
    """Fallback TTS using pyttsx3 (fully offline, no API key needed)."""
    try:
        import pyttsx3
        engine = pyttsx3.init()
        ...
        print(f"  [Based says]: {text}")
    except Exception as e:
        print(f"  pyttsx3 error ({e}) — text: {text}")


def speak(text, config):
    """Route TTS based on config: based cloud or local pyttsx3."""
...
def _handle_turn(audio, history, config):
    ...
    if backend == 'ollama':
        reply = generate_ollama(text, history, config)
    else:
        reply = generate_based(text, history, config)
    ...
    del history[:-10]   # keep last 10 turns
...
def main():
    ...
    print(f"  LLM backend : {llm}  |  TTS backend: {tts}")
    ...
    if engine == 'oww':
        ...
        print(f"\nBased Jarvis is live.  Say '{ww_label}' to activate.  Ctrl+C to quit.\n")
        try:
            while True:
        ...
        print("\nBased Jarvis is live.  Press ENTER to speak.  Ctrl+C to quit.\n")
        try:
            while True:
                input()
                _handle_turn(audio, history, config)
